...
def add_movie(movie_id):
    ...
    conn = psycopg2.connect(f"dbname=django_movies user=soware password={env('PASSWORD')}")
    cur = conn.cursor()
    ...
    sql = 'SELECT id, name FROM movies_job WHERE name IN %s'
    cur.execute(sql, (tuple(jobs),))
    jobs_in_db = cur.fetchall()
    print("Jobs in DB:", jobs_in_db)
    print("Jobs:", jobs)

    # Asegurémonos de que jobs_in_db tenga la estructura esperada
    jobs_in_db_names = [name for _, name in jobs_in_db]

    # Verificación
    print("Jobs in DB Names:", jobs_in_db_names)

    jobs_to_create = [(name,) for name in jobs if name not in jobs_in_db_names]
    print("Jobs to create:", jobs_to_create)
